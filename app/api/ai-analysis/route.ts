...
export async function POST(request: Request) {
  try {
    ...
    periodTransactions
      ...
      .forEach((t) => {
        ...
        expenseBreakdown[categoryName] =
          (expenseBreakdown[categoryName] || 0) + Number(t.base_amount);
      });
    ...
    periodTransactions
      ...
      .forEach((t) => {
        ...
        incomeBreakdown[categoryName] =
          (incomeBreakdown[categoryName] || 0) + Number(t.base_amount);
      });
    ...
    const monthlyAvgExpenses =
      ...
      Math.max(Object.keys(monthlyData).length, 1);
    const burnRate = monthlyAvgExpenses;
    const cashRunway =
      cashBalance > 0 && burnRate > 0 ? cashBalance / burnRate : null;
...
${
  Object.entries(incomeBreakdown)
    .map(
      ([cat, amount]) =>
        `- ${cat}: ${business.base_currency} ${amount.toFixed(2)}`
    )
    .join("\n") || "No income data"
}
...
${
  Object.entries(monthlyData)
    .map(
      ([month, data]) =>
        `${month}: Income ${data.income.toFixed(2)}, Expenses ${data.expenses.toFixed(2)}, Profit ${data.profit.toFixed(2)}`
    )
    .join("\n") || "No monthly data"
}
    ...
    try {
      await supabase.from("ai_analysis_history").insert({
        ...
        financial_snapshot: {
          ...
          breakdowns: financialData.breakdowns,
        },
      });
    } catch (saveError) {
      console.error("[AI_ANALYSIS_SAVE_ERROR]", saveError);
