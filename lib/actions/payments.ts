...
export async function getPayments(filters: { status?: 'pending' | 'paid' | 'overdue' } = {}) {
...
export async function getPaymentSummary() {
  ...
  const upcoming = payments.filter(p => new Date(p.transaction_date) >= new Date());

  const nextPayment = upcoming.length > 0
    ? upcoming.sort((a, b) => new Date(a.transaction_date).getTime() - new Date(b.transaction_date).getTime())[0]
    : null;
...
export async function createPayment(payment: any) {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      base_amount: payment.amount, // Simplified
    })
    .select()
    .single()
